...
get_distribution() {
    ...
    # Check if there's an upstream release file
    if [ "$lsb_dist" = "linuxmint" ] && [ -r /etc/upstream-release/lsb-release ]; then
        lsb_dist="$(. /etc/upstream-release/lsb-release && echo "$DISTRIB_ID" | tr '[:upper:]' '[:lower:]')"
